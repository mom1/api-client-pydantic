...
class ParamsSerializer:
    ...
    def _get_param_type(self, arg: inspect.Parameter) -> Any:
        ...
        if annotation == self.signature.empty:
            annotation = str if arg.default == self.signature.empty else type(arg.default)

        if annotation is type(None) or annotation is type(Ellipsis):
            annotation = str
...
class ResponseSerializer:
    ...
    def __init__(self, response: Optional[Type[BaseModel]] = None) -> None:
...
def serialize(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        return serialize_response(response=response)(result_func)
